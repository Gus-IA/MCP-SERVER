...
class ToolManager:
    ...
    async def get_all_tools(cls, clients: dict[str, MCPClient]) -> list[dict[str, Any]]:
        """Gets all tools from the provided clients, formatted for OpenAI tool calling."""
    ...
    async def execute_tool_requests(
        cls, clients: dict[str, MCPClient], tool_calls: list[Any]
    ) -> List[dict[str, Any]]:
        """Executes OpenAI tool calls against the provided clients."""
        ...
        for tool_call in tool_calls:
            if isinstance(tool_call, dict):
                ...
                tool_input_raw = fn.get("arguments")
            else:
